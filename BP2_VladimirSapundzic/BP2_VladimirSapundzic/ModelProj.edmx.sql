...
-- --------------------------------------------------
-- Date Created: 06/01/2021 10:16:49
-- Generated from EDMX file: C:\Users\Vlada\Desktop\BazePodataka2\BP2_VladimirSapundzic\BP2_VladimirSapundzic\ModelProj.edmx
-- --------------------------------------------------
...
-- Creating foreign key on [KupacIdKor] in table 'RecenzijaSet'
ALTER TABLE [dbo].[RecenzijaSet]
ADD CONSTRAINT [FK_KupacRecenzija]
    FOREIGN KEY ([KupacIdKor])
...
-- Creating foreign key on [KnjigaISBN] in table 'OsvajaSet'
ALTER TABLE [dbo].[OsvajaSet]
ADD CONSTRAINT [FK_KnjigaOsvaja]
    FOREIGN KEY ([KnjigaISBN])
    REFERENCES [dbo].[KnjigaSet]
        ([ISBN])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_KnjigaOsvaja'
CREATE INDEX [IX_FK_KnjigaOsvaja]
ON [dbo].[OsvajaSet]
    ([KnjigaISBN]);
GO

-- Creating foreign key on [AdministratorIdKor] in table 'KreiraSet'
ALTER TABLE [dbo].[KreiraSet]
ADD CONSTRAINT [FK_KreiraAdministrator]
    FOREIGN KEY ([AdministratorIdKor])
...
-- Creating foreign key on [KupacIdKor] in table 'KupovinaSet'
ALTER TABLE [dbo].[KupovinaSet]
